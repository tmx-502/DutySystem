$(function ($) {
    var storage, fail, uid; try { uid = new Date; (storage = window.localStorage).setItem(uid, uid); fail = storage.getItem(uid) != uid; storage.removeItem(uid); fail & config - fixed - footer & (storage = false); } catch (e) { }
    if (storage) {
        try {
            var usedSkin = localStorage.getItem('config-skin'); if (usedSkin != '') {
                $('#skin-colors .skin-changer').removeClass('active'); $('#skin-colors .skin-changer[data-skin="' + usedSkin + '"]').addClass('active');
                if (usedSkin == "theme-navyBlue") {
                    $("#logo img").attr("src", "../Image/blacklogo.png");
                    $("#menuwest").removeClass("blackred");
                    $("#menuwest").addClass("black");
                    
                } else if (usedSkin == "theme-amethyst") {
                    $("#logo img").attr("src", "../Image/purplelogo.png");
                    $("#menuwest").removeClass("blackred");
                    $("#menuwest").addClass("black");
                } else if (usedSkin == "theme-red") {
                    $("#logo img").attr("src", "Image/whitelogo.png");
                    $("#menuwest").removeClass("black");
                    $("#menuwest").addClass("blackred");
                } else if (usedSkin == "theme-blue-gradient") {
                    $("#logo img").attr("src", "Image/whitelogo.png");
                    $("#menuwest").removeClass("blackred");
                    $("#menuwest").addClass("black");
                } else {
            ...
            var fixedLeftmenu = localStorage.getItem('config-fixed-leftmenu'); if (fixedLeftmenu == 'fixed-leftmenu') {
                $('body').addClass(fixedLeftmenu); $('#config-fixed-sidebar').prop('checked', true); if ($('#page-wrapper').hasClass('nav-small')) { $('#page-wrapper').removeClass('nav-small'); }
    ...
    $('#config-tool-cog').on('click', function () { $('#config-tool').toggleClass('closed'); }); $('#config-fixed-header').on('change', function () {
        ...
        var boxedLayout = ''; if ($(this).is(':checked')) { $('body').addClass('boxed-layout'); boxedLayout = 'boxed-layout'; }
        ...
        var rtlLayout = ''; if ($(this).is(':checked')) { rtlLayout = 'rtl'; }
        else { }
    ...
    }); if (!storage) { $('#config-fixed-header').prop('checked', false); $('#config-fixed-footer').prop('checked', false); $('#config-fixed-sidebar').prop('checked', false); $('#config-boxed-layout').prop('checked', false); $('#config-rtl-layout').prop('checked', false); }
